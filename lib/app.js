...
app.post('/api/strongest', async (req, res) => {
  try {
    ...
    VALUES ($1, $2, $3, $4, $5, $6)
    RETURNING id, name, type, description, power, is_good as "isGood", user_id as "userId";
    `, [being.name, being.type, being.description, being.power, being.isGood, being.userId]);
...
app.put('/api/strongest/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      WHERE id = $7
      RETURNING id, name, type, description, power, is_good as "isGood", user_id as "userId";
    `, [being.name, being.type, being.description, being.power, being.isGood, being.userId, req.params.id]);
